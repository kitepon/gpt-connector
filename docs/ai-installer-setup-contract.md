...
Chromeの表示状態も変えず、`auth_required`を含む診断JSONをそのまま返す。
...
次の正規入口で専用Chromeを表示する。

```bash
gpt-connector browser show
```

表示成功後に停止し、そのChromeでChatGPTへログインするよう人間へ依頼する。ログイン完了の申告後、
`doctor`を再実行する。`browser show`は正規専用profileとCDP endpointの所有者を検査し、別profileや別PIDを表示しない。
...
| `auth_required` | `gpt-connector browser show`で専用Chromeを表示し、成功後に手動ログインを人間へ依頼して完了申告まで停止する。 |
