...
続く公開版0.12.1でも同じSSH経路から`browser start --provider grok`が`already_ready`、`browser show --provider grok`が`shown`になり、対話taskと一時記録は残らなかった。`grok-doctor`は`ready`。`expert`と`auto`の短文Chatはいずれも回答まで成功した。mode別の回答記録は[Grok実測](../grok/observations.md)に記す。
